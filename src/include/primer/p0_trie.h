  ...
  bool Insert(const std::string &key, T value) {
    if (!key.empty()) {
      ...
      if (last_node == nullptr) {
        ...
        auto new_last_node = new TrieNodeWithValue(std::move(**node), value);
        node->reset(new_last_node);
        result = true;
      } else if (!last_node->get()->IsEndNode()) {
        auto new_last_node = new TrieNodeWithValue<T>(std::move(**last_node), value);
        last_node->reset(new_last_node);
        result = true;
      } else {
        result = false;
      }
    ...
    if (!key.empty()) {
      ...
      if (!node->get()->HasChildren()) {
        for (int i = static_cast<int>(path.size() - 1); i >= 0; i--) {
          ...
          parent->get()->RemoveChildNode(key[i]);
          node = parent;
        }
      } else {
